...
export function CostPerStepChart({ snapshots, currentStep }: CostPerStepChartProps) {
  const data = snapshots.map((s) => ({
    step: s.stepIndex,
    cachedInput: s.cost.cachedInput,
    cacheWrite: s.cost.cacheWrite,
    uncachedInput: s.cost.uncachedInput,
    output: s.cost.output,
    compaction: s.cost.compactionInput + s.cost.compactionOutput,
  }))

  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <h3 className="mb-3 text-sm font-medium">Cost per Step</h3>
